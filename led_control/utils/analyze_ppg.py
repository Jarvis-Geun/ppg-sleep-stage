...
def analyze_ppg(ppg, minute):
    ...
    ########## 변경할 것 !!! ##########
    f = open("data/hju1/hju1_HR_{}.txt".format(minute), "w")
    for i in range(heart_rate.shape[0]):
        f.write(str(heart_rate[i]) + '\n')

    return heart_rate_ts.max() - heart_rate_ts.min(), PPG_nni.mean(), heart_rate.mean()


if __name__=="__main__":
    # ppg = np.loadtxt("40min_data/geun/geun_ppg_split.txt")
    ppg = np.loadtxt("40min_data/geun/geun_ppg_split.txt")
    heart_rate_ts, nni, heart_rate = analyze_ppg(ppg)
    print(heart_rate)
